...
import com.example.fypdeadreckoning.helpers.extra.LatLon
...
class MovementTestActivity : AppCompatActivity(), SensorEventListener {
    ...
    private var gyroCIntegration = GyroscopeDeltaOrientation(0f, null)
    // private var gyroUIntegration = GyroscopeDeltaOrientation(0f, null)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initialDirection =
            magneticFieldOrientation.getDirection(gravityValues, magValues, FloatArray(3))
    ...
    // TODO Find formula for calculating margin for error
    // Calculate the margin for error between GPS and DR
    private fun calcMarginForError(positions: ArrayList<LatLon>): Double {
        var total = 0.0
        for (i in 1 until positions.size) {
            total += ExtraFunctions.haversine(
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (!isHeadingInitialized && gravityCount >= 5 && magCount >= 5) {
            ...
            gyroUOrientation = GyroscopeEulerOrientation(ExtraFunctions.IDENTITY_MATRIX)
            gyroCIntegration = GyroscopeDeltaOrientation(0f, null)
